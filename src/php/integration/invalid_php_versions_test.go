...
var _ = Describe("invalid PHP versions", func() {
	...
	It("version is specified using .bp-config/options.json", func() {
		app = cutlass.New(filepath.Join(bpDir, "cf_spec", "fixtures", "invalid_php_version"))
	...
	It("version is specified using composer.json", func() {
		app = cutlass.New(filepath.Join(bpDir, "cf_spec", "fixtures", "invalid_php_version_composer"))
		app.SetEnv("COMPOSER_GITHUB_OAUTH_TOKEN", os.Getenv("COMPOSER_GITHUB_OAUTH_TOKEN"))
